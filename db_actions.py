...
class MyDB(object):
    ...
    def db_fetch_all(self, string):
        try:
            ...
            logging.error("[db_actions][db_fetch_all] Exception - {}".format(
                e))
        finally:
            cur.close()
    ...
    def checked_out_by(self, device_id):
        ...
        user_info = self.db_fetch(
            "SELECT CheckedOutBy from Devices WHERE DeviceID = {}".format(int(
                device_id)))
        logging.debug(
            "[db_actions][checked_out_by] DeviceID {} checked out by {}".format(
    ...
    def get_port_from_device_id(self, device_id):
        ...
        try:
            ...
            logging.debug(
                "[db_actions][get_port_from_device_id] No port registered for device {}".format(
                    device_id))
    ...
    def user_info(self, user_input):
        ...
        try:
            ...
            logging.info(
                "[db_actions][user_info] UserID input. Checked out by {}".format(
                    user_info))
            return user_info
        except:
            try:
                user_info = self.db_fetch(
                    "SELECT * from Users WHERE FirstName = '{}' AND LastName = '{}'".format(
                        str(user_input[0]), str(user_input[1])))
                print("Your UserID is: {}".format(user_info.get("UserID")))
                logging.info(
                    "[db_actions][user_info] User name input. Checked out by {}".format(
                        user_info))
                return user_info
            except:
                return None
    ...
    def update_time_reminded(self, device_name):
        ...
        """
        self.db_commit(
            "UPDATE Devices set LastReminded = unix_timestamp() where DeviceName = '{}'".format(
                device_name))
        logging.info(
    ...
    def add_to_database(self, device_info):
        ...
        """
        self.db_commit(
            "INSERT INTO Devices(DeviceName,Manufacturer,Model,Type,OS,DeviceID,SerialUDID,Port,CheckedOutBy,"
            "TimeCheckedOut,LastReminded) VALUES('{}','{}','{}','{}','{}','{}','{}','{}','0','0','40000')".format(
                device_info[0], device_info[1], device_info[2], device_info[
    ...
    def check_in(self, device_id, port):
        ...
        try:
            self.db_commit("UPDATE Devices set CheckedOutBy = '0',"
                           "Port = '{}' where DeviceID = {}".format(port,
                                                                    device_id))
        except Exception as e:
    ...
    def check_out(self, user_id, device_id):
        ...
        try:
            self.db_commit(
                ...
                "TimeCheckedOut = unix_timestamp(), LastReminded = unix_timestamp() where DeviceID = {}".format(
                    user_id, device_id))
        except Exception as e:
            logging.debug(
